...
int32_t usbTransfer(const char * func, uint8_t type, uint8_t* buf, int len)
{
    ...
    if (ret < 0) {
        fprintf(stderr, "%s: Failed to %s %d bytes '%s'\n", func,
                (type == BULK_WRITE_ENDPOINT) ? "write" : "read", len, libusb_strerror(ret));
        return -1;
    }
    return transfered;
}
...
int32_t ch341SpiRead(uint32_t add, uint8_t *buf, int32_t len)
{
    ...
    const int32_t max_payload = CH341_MAX_PACKET_LEN - CH341_PACKET_LENGTH - CH341_MAX_PACKETS + 1 - 4;
    int32_t tmp, pkg_len, pkg_count;
...
int32_t ch341SpiWrite(uint32_t add, uint8_t *buf, int32_t len)
{
    uint8_t out[WRITE_PAYLOAD_LENGTH];
    uint8_t in[CH341_PACKET_LENGTH];
    int32_t tmp, pkg_count;
    struct libusb_transfer *xferBulkIn, *xferBulkOut;
    uint32_t idx = 0;
    int32_t ret;
	int32_t tlen = len;
